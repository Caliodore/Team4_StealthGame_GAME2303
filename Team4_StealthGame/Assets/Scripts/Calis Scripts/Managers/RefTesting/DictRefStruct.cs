...
public class DictRefStruct : MonoBehaviour
{
    ...
    {
        //Custom class for taking in MonoBehaviour and outputting name of script and the name of the object using it.
    ...
    public class ObjectGroup<T> where T : MonoBehaviour
    {
        ...
        { 
            ...
            objScriptName = objType.name;
            gameObjRef = objType.GetComponent<Transform>().gameObject;
            if(gameObjRef == null)
            { 
                gameObjRef = objType.GetComponentInParent<Transform>().gameObject;
                if(gameObjRef == null)
                    print($"This Object Group is not attached to a GameObject.");
            }
        }
